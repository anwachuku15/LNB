...
const NotificationsScreen = (props) => {
  ...
  let notifications = useSelector((state) =>
    state.auth.notifications.filter(
      (notification) => notification.type !== "connection request"
    )
  );
  ...
  let connectReqs = useSelector((state) =>
    state.auth.notifications.filter(
      (notification) => notification.type === "connection request"
    )
  );
  connectReqs = connectReqs.sort((a, b) =>
    a.timestamp > b.timestamp ? -1 : 1
  );
    ...
    <TouchableCmp
      onPress={() => {
        item.type === "new connection" &&
          navToUserProfile(item.senderId, item.senderName);
        item.type === "likeNeed" &&
          navToNeed(item.needId, item.senderName, item.type);
      ...
      <ListItem
        ...
        title={
          ...
          >
            ...
            >
              {moment.utc(new Date(item.timestamp)).fromNow()}
            </Text>
          </View>
        }
    ...
    connectReqs.length > 0 && (
      ...
      >
        <View style={styles.requestsContainer}>
          ...
          <MaterialIcons name="navigate-next" color={Colors.blue} size={24} />
        </View>
      </TouchableCmp>
    );
