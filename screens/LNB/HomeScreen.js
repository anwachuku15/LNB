...
const HomeScreen = (props) => {
  ...
  const registerForPushNotificationsAsync = async () => {
    if (Constants.isDevice) {
      ...
      try {
        ...
        db.doc(`/users/${userId}`).set({ pushToken: token }, { merge: true });
      } catch (err) {
  ...
  const authPosts = useSelector((state) =>
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      // TODO: ALL LISTENERS FROM authActions.js:664
      // TODO: On new needs snapshot, indicate new needs button, then refresh/add to state
    } catch (err) {
      console.log(err);
...
const TouchableHeader = () => {
  ...
  return (
    ...
    >
      <View style={{ width: WINDOW_WIDTH * 0.6, alignItems: "center" }}>
        <Image
