...
const DirectoryScreen = (props) => {
  ...
  const connectReqHandler = useCallback(
    async (item) => {
      try {
        await dispatch(connectReq(authId, authName, item.userId));
        requestedUsers.push(item.userId);
      ...
      >
        ...
        >
          ...
          {search.length > 0 && (
            ...
            >
              <MaterialIcons name="cancel" size={20} color={Colors.disabled} />
            </TouchableCmp>
          )}
        </View>
      </View>
