...
const MessagesScreen = (props) => {
  ...
  const loadChats = useCallback(async () => {
    try {
      ...
      let userChats = [];
      const groupChats = (await db.doc(`/users/${uid}`).get()).data()
        .groupChats;
      const allChats = await (
        await db
          .collection("chats")
        ...
        if (doc.id.includes(uid)) {
          ...
          const authUser = uid < chatWithId ? "user1" : "user2";
          const name =
            uid > chatWithId
          ...
          if (doc.data().messageCount > 0) {
            userChats.push({
              ...
              lastRead:
                authUser === "user1"
                  ? doc.data().lastRead.user1.timestamp
                  : doc.data().lastRead.user2.timestamp,
            });
          }
  ...
  const renderChat = ({ item }) => (
    ...
    >
      <ListItem
        ...
        containerStyle={{
          backgroundColor:
            item.lastRead < item.lastMessageTimestamp
              ? "rgba(251, 188, 4, 0.4)"
              : background,
        }}
        title={
          ...
          >
            ...
            >
              {moment.utc(new Date(item.lastMessageTimestamp)).fromNow()}
            </Text>
          </View>
        }
        subtitle={
      ...
      <SafeAreaView style={{ backgroundColor: background, ...styles.screen }}>
        <View
          style={{
            ...
            ...{ marginHorizontal: 15, marginTop: 10, alignSelf: "center" },
          }}
        >
          <View style={{ justifyContent: "center" }}>
            <Feather name="search" size={14} color={Colors.placeholder} />
          </View>
          <TextInput
            ref={searchInput}
            autoFocus={false}
          ...
          {search.length > 0 && (
            ...
            >
              <MaterialIcons name="cancel" size={16} color={Colors.disabled} />
            </TouchableCmp>
          )}
        </View>
          ...
          <DismissKeyboard>
            <View style={{ flex: 1, alignItems: "center", paddingTop: 10 }}>
              <Text style={{ color: Colors.placeholder }}>
                Search for someone you'd like to message
              </Text>
            </View>
          </DismissKeyboard>
        )}
