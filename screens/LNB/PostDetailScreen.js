...
const PostDetailScreen = (props) => {
  ...
  const need = useSelector((state) =>
    state.posts.allNeeds.find((need) => need.id === needId)
  );
  ...
  const commentAvatar = (item) => {
    return <Avatar source={{ uri: item.userImage }} />;
  };
  ...
  const loadComments = useCallback(async () => {
    ...
    try {
      ...
      const allComments = await (
        await db.collection("comments").orderBy("timestamp", "asc").get()
      ).docs.forEach((doc) => {
  ...
  const renderComment = ({ item }) => (
    <TouchableCmp onPress={() => {}}>
      <View
        style={{ ...styles.commentView, ...{ backgroundColor: background } }}
        key={item.commentId}
      >
        <TouchableCmp
        ...
        <View
          style={{
            flex: 1,
            backgroundColor: scheme === "light" ? "#EEEEEE" : "#414141",
            padding: 10,
        ...
        >
          ...
          <Hyperlink linkDefault={true} linkStyle={{ color: Colors.blue }}>
            <Text
              style={{
                marginTop: 12,
                fontSize: 14,
  ...
  return (
    <View style={{ backgroundColor: background, ...styles.screen }}>
          ...
          ListEmptyComponent={() => (
            ...
            >
              <Text style={{ color: Colors.placeholder }}>
                Be the first to comment on {need.userName}'s need!
              </Text>
            </View>
          )}
          ListHeaderComponent={() => (
            <View
              style={{
                ...
                ...{
                  backgroundColor:
                    scheme === "dark" ? Colors.darkSearch : "white",
                },
              }}
            >
              <TouchableCmp
              ...
              >
                <Image
                  source={{ uri: need.userImage, cache: "force-cache" }}
                  style={styles.avatar}
                />
                {/* {MemoizedAvatar} */}
              </TouchableCmp>
      ...
      <KeyboardAvoidingView
        behavior="padding"
        keyboardVerticalOffset={Platform.select({ ios: 85, android: 500 })}
      >
        <View
          style={{
            flexDirection: "row",
...
PostDetailScreen.navigationOptions = (navData) => {
  ...
  const postType = navData.navigation.state.params.postType;
  ...
  return {
    ...
    // ),
    headerTitle: () => (
      <Text style={styles.headerTitle}>{postTypeCapitalized}</Text>
    ),
    headerTitleStyle: {
