...
const UserProfileScreen = (props) => {
  ...
  const user = useSelector((state) =>
    state.auth.allUsers.find((user) => user.userId === userId)
  );
  ...
  const userPosts = useSelector((state) =>
    state.posts.allNeeds.filter((need) => need.uid === userId)
  );
  ...
  const disconnectHandler = (authId, selectedUserId) => {
    Alert.alert(
      "Disconnect",
      "Are you sure you want to disconnect from " + user.displayName + "?",
      [
        {
          text: "No",
          style: "cancel",
  ...
  const WebsiteIcon = () => (
    <View>
      {user.website.includes("linkedin.com") && (
        <TouchableCmp
          onPress={() => handleOpenLink(user.website)}
      ...
      {!user.website.includes("linkedin.com") &&
        !user.website.includes("instagram.com") && (
          <TouchableCmp
            ...
            style={{ justifyContent: "center" }}
          >
            <Ionicons name="md-link" size={24} color={Colors.blue} />
          </TouchableCmp>
        )}
    </View>
  );
  ...
  const EditProfileButton = () => (
    <View style={{ alignSelf: "center" }}>
      <TouchableCmp
        onPress={() => {
          props.navigation.navigate("editProfileModal");
        }}
        style={{
  ...
  const LightboxView = ({ navigator }) => (
    <Lightbox
      navigator={navigator}
      backgroundColor={
        scheme === "dark" ? Colors.darkHeader : Colors.lightHeader
      }
      underlayColor="rgba(255, 255, 255, 0.1)"
      springConfig={{ tension: 10, friction: 7 }}
      renderHeader={(close) => (
        <TouchableCmp onPress={close} style={styles.closeButton}>
          <Ionicons name="ios-close" size={36} color={Colors.placeholder} />
        </TouchableCmp>
      )}
      renderContent={() => (
        <Image
            ...
            ListEmptyComponent={() => (
              ...
              >
                <Text style={{ color: Colors.placeholder }}>
                  {user.displayName} hasn't posted any needs.
                </Text>
              </View>
            )}
                  ...
                  >
                    ...
                    <View style={{ alignItems: "center", marginBottom: 10 }}>
                      ...
                      <View style={styles.avatarContainer}>
                        <TouchableWithoutFeedback
                          onPress={() =>
                            props.navigation.push("UserProfilePicture", {
                              uri: user.imageUrl,
                              user: user,
                            })
                          }
                      ...
                      <View>
                        <View
                          style={{
                            ...
                            paddingTop: 8,
                            paddingBottom: accept ? 0 : 15,
                          }}
                        >
                          {outgoingRequests.includes(user.userId) && (
                            <TouchableCmp
                              onPress={() => {
                                unrequestHandler(authUser.userId, userId);
                              }}
                              style={{
                          ...
                          {userConnectionIds.includes(user.userId) && (
                            <TouchableCmp
                              onPress={() => {
                                disconnectHandler(authUser.userId, userId);
                              }}
                              style={{
                                ...styles.connectButton,
                                borderColor: Colors.primary,
                        ...
                        {incomingRequests.includes(user.userId) && (
                          ...
                          >
                            {user.displayName} has requested to connect with
                            you.
                          </Text>
                        )}
                      </View>
                    )}
                    {userId === authUser.userId && <EditProfileButton />}
                  </View>
                  {/* PINNED NEED */}
                  {pinned && (
                    <View style={{}}>
    ...
    headerRight: () => (
      <HeaderButtons HeaderButtonComponent={HeaderButton}>
        <Item
          ...
          onPress={() => {
            uid === userId ? navData.navigation.navigate("Settings") : {};
          }}
        />
      </HeaderButtons>
    ),
