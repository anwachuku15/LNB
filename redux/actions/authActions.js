...
export const MARK_MESSAGE_NOTIFICATIONS_READ =
  "MARK_MESSAGE_NOTIFICATIONS_READ";
export const MARK_CONNECT_NOTIFICATIONS_READ =
...
export const createProfile = (uri, headline, bio) => {
  return async (dispatch, getState) => {
    ...
    const imgUrl =
      imageUrl === "don't update" ? auth.credentials.imageUrl : imageUrl;
...
export const updateProfile = (headline, location, bio, link, uri) => {
  return async (dispatch, getState) => {
    ...
    const website =
      link.trim() !== "" && link.trim().substring(0, 4) !== "http"
        ? `https://${link.trim()}`
        : link;
    await db
      .doc(`/users/${auth.userId}`)
...
export const getAuthenticatedUser = (
  ...
  return async (dispatch) => {
    try {
      ...
      await (await db.collection("chats").get()).docs.forEach((doc) => {
        if (doc.id.includes(userId)) {
          if (
            doc.data().lastRead.user1 &&
            doc.data().lastRead.user1.uid === userId
          ) {
            ...
            doc.data().lastRead.user2 &&
            doc.data().lastRead.user2.uid === userId
          ) {
            lastReadMessages.push({
              chatId: doc.id,
      ...
      // TODO: move these to HomeScreen.js:229 in loadData callback and remove constants
      const newConnectionListener = db
      ...
      const requestsListener = db
        ...
        .then((doc) => {
          const requestListener = doc.ref.onSnapshot((snapshot) => {
            ...
            dispatch(
              updateIncomingRequests(snapshot.data().pendingConnections)
            );
          });
        })
        .catch((err) => console.log(err));

      const unreadListener = db
...
export const pinNeed = (needId, uid) => {
  db.collection("needs")
    ...
    .then((doc) => {
      if (!doc.empty) {
        doc.docs[0].ref.set({ isPinned: false }, { merge: true });
      }
    })
    .then(() => {
      db.doc(`/needs/${needId}`).set({ isPinned: true }, { merge: true });
    })
    .catch((err) => console.log(err));
};
      ...
      db.doc(`/users/${selectedUserId}`)
        ...
        .then(() => {
          db.doc(`/users/${selectedUserId}`)
            ...
            .then(async () => {
              const pushToken = (
                await db.doc(`/users/${selectedUserId}`).get()
              ).data().pushToken;
              if (pushToken) {
...
export const confirmConnect = (
  ...
  return async (dispatch, getState) => {
    ...
    db.doc(`/connections/${connectionId}`)
      .set({
    ...
    sendConnectionNotification(
      ...
      selectedUserData.headline,
      selectedUserData.pushToken
    );
    // dispatch({
    //     type: SET_PENDING_CONNECTIONS,
...
const sendConnectionNotification = (
  authId,
  authName,
  authImg,
  authHeadline,
...
export const setNotifications = () => {
  return async (dispatch, getState) => {
    ...
    await (
      await db
        .collection("notifications")
        .where("recipientId", "==", getState().auth.userId)
        .get()
    ).forEach((doc) => {
...
export const markNotificationsAsRead = () => {
  return async (dispatch, getState) => {
    let batch = db.batch();
    const unreadNotificationIds = getState()
      .auth.notifications.filter((notification) => !notification.read)
...
export const markMessageNotificationsAsRead = () => {
  return async (dispatch, getState) => {
    let batch = db.batch();
    const unreadNotificationIds = getState()
      .auth.messageNotifications.filter((notification) => !notification.read)
      .map((notification) => notification.id);
    unreadNotificationIds.forEach((id) => {
      const notification = db.doc(`notifications/${id}`);
      batch.update(notification, { read: true });
...
export const markConnectNotificationsAsRead = () => {
  return async (dispatch, getState) => {
    let batch = db.batch();
    const unreadNotificationIds = getState()
      .auth.connectNotifications.filter((notification) => !notification.read)
      .map((notification) => notification.id);
    unreadNotificationIds.forEach((id) => {
      const notification = db.doc(`notifications/${id}`);
      batch.update(notification, { read: true });
    ...
    if (uid < selectedUserId) {
      ...
      db.doc(`chats/${chatId}`)
        ...
        .then((doc) => {
          ...
          let existingChat = lastReadMessages.find(
...
export const disconnect = (authId, selectedUserId) => {
  return async (dispatch, getState) => {
    ...
    notifications.forEach((doc) => {
      if (
        users.includes(doc.data().recipientId) &&
        users.includes(doc.data().senderId)
      ) {
    ...
    await db
      .doc(`/users/${authId}`)
      .set({ connections: authData.connections - 1 }, { merge: true });
    await db
      .doc(`/users/${selectedUserId}`)
      .set({ connections: selectedUserData.connections - 1 }, { merge: true });
    dispatch({
      type: SET_USER,
      credentials: {
