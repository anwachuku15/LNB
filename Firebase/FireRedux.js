...
class FirePostData {
    ...
    createNeedMedia = (timestamp, postId, uid, userImage, userName, body, media, remoteUri, taggedUsers) => {
        db.collection('needs')
            .add({
                userName,
                body,
    ...
    createNeed = (timestamp, postId, uid, userImage, userName, body, taggedUsers) => {
        db.collection('needs')
            .add({
                userName,
                body,
    ...
    addComment = (newComment, commentId, needData, postId, needUserId, authUserId, authUserName, authUserImage) => {
            ...
            .then(() => {
                ...
                .then(async (doc) => {
                    ...
                    if (needUserId !== authUserId) {
                        ...
                        this.sendCommentNeedNotification(postId, needUserId, pushToken, authUserId, authUserName, authUserImage)
                    }
                })
            ...
            .then(data => {
                ...
                commenters.forEach(async uid => {
                    if ((uid !== needUserId) && (uid !== authUserId)) {
                        ...
                        this.sendCommentNotificationBatch(postId, needData.userName, uid, pushToken, authUserId, authUserName, authUserImage)
                    }
                })
            })
            .catch(err => {
    ...
    sendCommentNeedNotification = (needId, recipientId, pushToken, authId, authName, authImage) => {
    ...
    sendCommentNotificationBatch = (needId, needUserName, recipientId, pushToken, authId, authName, authImage) => {
        db.collection('notifications').add({
            timestamp: new Date().toISOString(),
            type: 'commentThread',
            needId,
